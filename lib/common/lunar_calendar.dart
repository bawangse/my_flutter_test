...
/**农历的工具类
测试数据：2020年12月5日转公历结果为：2021年1月17日
Text('${LunarCalendarUtil.lunarToSolar(2020, 12, 5)}'),
 **/
class LunarCalendarUtil {
  ...
  /// @param monthDay   农历日
  /// 我将下面的参数放到代码中自动调用了
  /// @param isLeapMonth   该月是否是闰月(该参数可以根据本类中 leapMonth()方法，先判断一下要查询的年份是否有闰月，并且闰的几月).
  ///
  /// @return 返回农历日期对应的公历日期，year0, month1, day2.
  static List<int> lunarToSolar(int year, int month, int monthDay) {
    bool isLeapMonth = LunarCalendarUtil.leapMonth(year) == 0 ? false : true;
    int dayOffset;
    int leapMonth;
    int i;
    if (year < MIN_YEAR ||
    ...
    List<int> solarInfo = List.filled(3, 0);
    for (i = 1; i < 13; i++) {
